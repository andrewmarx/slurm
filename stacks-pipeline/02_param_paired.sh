...
num_samples=$(find ./output/02_param/samples/ -name "*.1.fq.gz" | wc -l)
...
for M in ${M[*]}; do
  for m in ${m[*]}; do
    f_counter=0
    for data_file in ./output/02_param/samples/*.1.fq.gz; do
      counter=\$((counter + 1))
...
then
  ...
  ustacks -f \$data_file -o \$outdir -i \$f_counter -M \${M} -m \${m} -p ${ustacks_cores} --name \${outname}
  retval=\$?
fi
